...
//			printf("dist %d %d\n", (int)((CELL / (double)w_ray.distance.x) * wolf->map.cam.screendist), w_ray.distance.y);
		wall->col = (w_ray.distance.x < w_ray.distance.y) ? (int)round((CELL / (double)w_ray.distance.x) * wolf->map.cam.screendist * 2 / 3) : (int)round((CELL / (double)w_ray.distance.y) * wolf->map.cam.screendist * 2 / 3); // wrong formula it seems
	}
	return (w_ray.inmap);
}
...
//		printf("%f\n", rays);
		...
		walls.wall->orientation = wolf->map.cam.orientation;
		if (rays == 90)
		{
			if(x)
